...
//! with SBR clear, which is why internet radio never saw any of this.
...
pub(crate) fn core_layer_config(asc: &[u8]) -> Option<Box<[u8]>> {
    ...
    // it would play wrong. All three bits must be *present* though: a config too short to hold
    // them is one Symphonia's reader refuses ahead of the gate this exists to get past.
...
impl BitWriter {
    ...
    /// The config, its last byte zero-padded, which is what a decoder expects to find.
