...
//! - **`warn`** — a user could notice. Expected, self-recovering and unbounded goes to
//!   `debug` with a count instead; per occurrence it only teaches the reader to skim
//!   warnings (`player::stream_health`).
//! - **`debug`** — what the user did, as a narrative: one line per class of action at
//!   the seam every path funnels through, and **nothing on a timer, frame or
//!   keystroke**. The *action*, never the widget — a play/pause line at the button
//!   misses the shortcut, the tray, the media keys and Now Playing. `trace` is unused,
//!   hence [`VERBOSE_LEVEL`] stopping here.
//!
//! `services::crash_report` shares the directory so "Open log folder" hands over
//! everything at once; neither sweep recognises the other's names, so neither retires
//! its files or a user's.
//!
//! **One writer, by `services::single_instance`'s doing** — a second launch forwards
//! and returns before `install` runs. Nothing here defends the file though: an
//! unmakeable claim boots anyway, and two Melodias would interleave with a rotation
//! able to race. Per-process files would trade the one artifact a reporter can hand
//! over for a folder they can't.
...
/// …and with it on. Not `trace`: there are no `log::trace!` sites in the tree, so it
/// would differ only in what the muted dependencies say.
const VERBOSE_LEVEL: &str = "debug";

/// The two dependency modules that warn about something we already know.
///
/// `layer3`'s bit-reservoir underflow fires once per *frame* on any stream not opened
/// at byte zero — every seek, every gapless preload — while the decoder compensates
/// and plays it fine. `sctk_adwaita::buttons` names controls on the client-side
/// titlebar winit builds and immediately hides in custom-titlebar mode, after KDE
/// answers the portal's `button-layout` with an empty left side.
///
/// Scoped by module, never by crate: each is the only `warn!` in its own, so the
/// siblings — the demuxer's, a glyph-rasterization failure — still land. Directives
/// match longest-name-first, so these outrank the floor. Shared by both specs, which
/// is why those are built rather than written twice: a verbose spec dropping `layer3`
/// buries the detail the switch was flipped for.
...
///
/// A bare `"info"` would floor every crate in the graph — slint, symphonia, zbus,
/// notify, reqwest — spending our rotation budget on their choices. Both tokens are
...
/// Whether `RUST_LOG` was **set** when [`install`] ran; if so it owns the spec and
/// [`set_verbose`] declines. Presence, not a successful parse — a malformed value
/// falls back to [`spec_for`], but it is still a developer saying which levels they
/// want, and a GUI switch quietly taking the spec back from a typo is the surprise.
static ENV_SPEC_WINS: OnceLock<bool> = OnceLock::new();

/// Start logging. Call once, as early in `main` as a [`Paths`] exists.
///
/// **Infallible on purpose.** Opening the file can fail for reasons that aren't the
/// app's — root-owned by one `sudo melodia`, a full disk, exhausted descriptors — and
/// a `?` refused to *start* Melodia over it, explaining why to the stderr a `.desktop`
/// launch discards. Degrades to stderr-only instead; crash reports survive either way
/// through the panic hook's plain `fs`.
///
/// Reads the Verbose Logging flag itself rather than taking it as an argument: applied
/// later from the UI, the whole boot would stay at [`NORMAL_LEVEL`], and a boot going
/// wrong is the window the switch is worth having.
pub fn install(paths: &Paths) {
    let _ = ENV_SPEC_WINS.set(std::env::var_os("RUST_LOG").is_some());

    // An unparseable settings file is no reason to start louder; it surfaces later
    // through `AppState::init`'s own read.
    ...
    let error = match start_to_file(paths, &spec) {
        ...
        // Not `to_string()`: every `FlexiLoggerError` arm is a static sentence and
        // `OutputIo` never interpolates its `io::Error`, so without the chain a
...
fn base_logger(spec: &str) -> Logger {
    // `try_with_env_or_str` falls back to `spec` on a malformed `RUST_LOG`, so only a
    // broken `spec_for` errors here. The `warn` floor costs the app's narrative, not
    // the process's logger; `default()` is `off()`, which would cost both.
    Logger::try_with_env_or_str(spec)
        .unwrap_or_else(|_| Logger::with(LogSpecification::warn()))
        // Adaptive rather than plain coloured: env_logger suppressed ANSI off a tty,
        // and piping the app somewhere shouldn't regress.
...
fn start_to_file(paths: &Paths, spec: &str) -> Result<LoggerHandle, FlexiLoggerError> {
    base_logger(spec)
        ...
        // Without this a restart truncates, so the run that crashed is gone by the
        // time the folder is opened — the one call whose absence defeats the feature
        // silently.
        .append()
        .rotate(
        ...
        // The default, stated because it is load-bearing: `process::exit(0)` runs no
        // destructors, and a buffered mode drops exactly the lines before a crash.
...
///
/// **`RUST_LOG` wins and this declines**, out loud rather than silently: it is the
/// developer escape hatch, and a GUI switch fighting it would make the variable mean
/// nothing from the moment Settings was opened.
pub fn set_verbose(on: bool) {
    ...
    match LogSpecification::parse(&spec) {
        ...
        // Unreachable while the spec test passes; keeping the current level beats
...
///
/// Best-effort: a diagnostics bundle missing a file is worth more than one that failed
/// to build, so an unreadable directory reads as no logs.
pub fn log_files() -> Vec<PathBuf> {
    let Some(handle) = HANDLE.get() else {
        return Vec::new();
    };
    // Two calls: asked together, the live file lands *after* the rotated ones and no
    // single ordering of that list is meaningful.
    let current =
...
/// whenever the live one is short. Split out to be testable without a logger.
