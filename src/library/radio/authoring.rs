...
/// station whose row already points at a logo is one [`super::logos::heal_station_logo`] skips,
/// so a *replacement* URL typed here would never land on its own. Every row an import creates is new
