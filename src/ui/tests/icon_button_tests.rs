...
/// The body `opener` opens, cut at its own closing brace.
///
/// A brace walk rather than a split on `\n    }`, because the depth of both blocks
/// this file reads is exactly what the second pin is about: cutting on the root's
/// indent, a re-nested glyph's body runs to `disc`'s closing brace instead of its
...
/// nested inside it. Nest the glyph back — which reads as tidier, the two sharing a
/// centre and a `press-scale` — and every value under 1.0 renders the disc to an
/// offscreen texture instead. `LibraryTabBand` is where that bites: it clips against
/// an *animating* height, which puts the layer's bounding rect on a clip that moves
/// every frame, i.e. `render_layer`'s allocate-a-fresh-GPU-texture branch rather
/// than its reuse one, for the length of every morph.
///
/// Nothing else covers it. `library_tab_band_tests` pins that the band fades the
/// disc through `IconButton`'s `fade` rather than an `opacity` of its own, which
/// stays true — and worthless — the moment the disc stops being childless.
#[test]
