...
fn tags(model: &VecModel<Row>) -> Vec<u32> {
    (0..model.row_count()).filter_map(|i| model.row_data(i).map(|r| r.tag)).collect()
}

fn labels(model: &VecModel<Row>) -> Vec<&'static str> {
    (0..model.row_count()).filter_map(|i| model.row_data(i).map(|r| r.label)).collect()
}
...
fn equal_rows_are_not_written_back() {
    ...
    apply_rows_keyed(&model, vec![row(1, "a", 9), row(2, "b", 9), row(3, "c", 9)], |r| r.id);
