...
//! * [`tracklist`] — the recency list: row actions, filter, column
//!   visibility, modifier-aware selection, and the header Shuffle pill.
...
/// its sortable siblings there is no `view_sort` entry — the list is mounted
/// non-sortable, so recency is the only order it has.
