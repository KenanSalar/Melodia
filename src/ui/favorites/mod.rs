...
//!
//! The hero's mosaic tiles are the first 4 *distinct* covers of the Most Played
//! tab's own walk — both rank through one shared clause
//! (`queries::track::MOST_PLAYED_ORDER`) rather than two hand-matched ones, so
//! they can't disagree about a tie — topped up from the favourites that tab
//! excludes once the played covers run out. Songs binds a `TrackList` whose
//! search *and* sort run in memory off [`crate::ui::track_list_cache`], the SQL
//! fetch returning the whole set once per `library_changed_tx` tick. Most Played
//! and Favorite Artists are virtualized grids over uncapped fetches; there is no
//! Favorite Albums tab, the Albums page covering that.
//!
//! Cache discipline is `src/ui/albums`': per-tier `CoverThumbs` LRUs released on
//! section leave, and for the two grids on tab-leave as well, so a hidden page's
//! resident footprint drops to nothing.
//!
...
//! seeding, `covers.rs` the four tiers, `rows.rs` the Slint models, `hero.rs`
//! the band, `songs.rs` the Songs tab, and `grids/` the two grid tabs in four
//! parts — `fetch`, `apply`, `warm` and `sort`.
...
///
/// `artists_ui` is the cross-tab open-artist hand-off, taken as a parameter so
/// the ordering is a compile error rather than a comment.
///
/// **The tab seed folds in here** rather than sitting with its siblings in
/// `hydrate_ui_from_settings`, because seeding writes the Slint property *and*
/// the handle's synchronous shadow — which the off-thread fetchers read to
/// decide which model to fill and which tier to warm. Folded, the handle is the
/// receiver rather than something that must still be in scope two functions on.
///
...
/// Rust-side state for the Favorites view, shared between the UI callbacks and
/// the async fetchers behind an `Arc`.
pub struct FavoritesUi {
    ...
    /// Synchronous shadow of `Favorites.tab-idx`. The off-thread fetchers pick
    /// which cover tier to warm from it — only one grid is ever mounted, so
    /// warming both is twice the resident buffers for nothing.
    active_tab: AtomicU8,
    /// [`SectionState`]'s dirty flag one level down, per fetch rather than per
    /// tab — `RecentlyPlayedUi::grid_dirty`'s shape, one page over.
    ///
    /// **The page has one `SectionActiveGate` for three mutually exclusive
    /// tabs**, so without these `kick_full_refresh` ran all three fetches every
    /// tick whatever was mounted: with a grid up, `refresh_tracks` still fetched
    /// every favourite, sorted it and converted the lot into rows that reached
    /// nothing, once per finished track. Two flags rather than three because one
    /// `refresh_grids` feeds both grid tabs.
    ///
    /// Gating is safe here only because `hero_chips::favorites_chips` is a
    /// per-tab match and `refresh_hero` — which answers the count, running time
    /// and mosaic on every tab — is never gated.
    ///
...
impl FavoritesUi {
    ...
    ///
    /// The section leave is the only caller, and it sits beside the `blur-img-*`
    /// wipe rather than in [`Self::release_section_state`] — that one bails when
    /// the user has already come back, where the wipe is unconditional and would
    /// leave the guard set against a hero with no blur left to guard. Every
    /// *other* move of the guard is made where the paint is.
    ...
    /// Drop every section-local buffer and clear the Slint models, so a hidden
    /// page holds nothing. Runs off the UI thread on section leave, which set
    /// `mark_dirty()` synchronously; no Rust-side cache is kept for the
    /// re-enter, the in-memory filter re-walking a freshly-fetched list anyway.
    ///
    /// Release order matches `AlbumsUi::release_section_state`: per-section LRUs
    /// first, then mutated state under the section gate, then a
    /// `heap_trim::trim` so glibc hands the freed pages back.
    ///
    /// The gate serializes those stores but does **not** order them, so it is
    /// not what stops a fetch resolving *after* the leave from repopulating what
    /// this emptied. That is each fetcher's own `section_active()` bail — one
    /// after the query, one after the prewarm's `.await` — plus the one inside
    /// each apply's event-loop closure.
    pub fn release_section_state(&self) {
        ...
        self.artist_thumbs.clear();
        {
            ...
            *self.inner.stats.lock() = FavoriteStats {
            ...
            // *wrong* rather than merely absent. `refresh_hero` is the shortest
            // of the three fetches, so it publishes first on re-enter and would
        ...
        // own `mark_dirty` two files away: all three sets are empty now, so the
        // next tab entered owes a fetch whatever the flags held on the way out.
    ...
    /// Track ids of the post-filter Songs tab, in display order — how
    /// `shuffle-all` and `play-row` recover ids without touching the Slint
    /// model, callers being off the UI thread.
    ...
    /// that grid rather than the Songs tab's list. Filtered through the same
    /// predicate `grids::apply::build_filtered_grids` builds the model with —
    /// the grid narrows with the search bar, and the raw cache would enqueue
    ...
    /// Flip `is_favorite` on a cached Songs-tab row so a single-row toggle
    /// lands without a re-fetch. `false` removes the row outright, for parity
    /// with a fetch that only ever returns `is_favorite = TRUE`.
    ...
    /// Set `rating` on a cached row. Unlike [`Self::flip_or_remove_track`],
    /// rating never affects membership, so the row is only patched.
