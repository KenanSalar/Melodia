...
//!
//! The Tracks view's fetch shape — one SQL fetch of the whole set per
//! `library_changed_tx` tick, with both the per-keystroke filter walk and the
//! per-header-click re-sort running in memory off
...
/// The active sort. Slint mirrors it in `Favorites.sort-*`, but this cache is
/// the source of truth — those properties are written from Rust, never back.
...
/// Update the cached sort. Callers follow it with [`resort_and_apply`] and a
/// `set_view_sort` persist, so the next launch lands on the same order.
...
/// Re-sort the Songs tab after a header click, entirely in memory: the set is
/// already resident and already covered, and only the display permutation moves,
/// so re-issuing the query would be an unbounded `SELECT` plus a full cover
/// prewarm for nothing.
...
/// Update the cached filter needle, so the live-refresh subscriber can re-walk
/// on a background thread without reading the Slint global. Folded on the way
/// in, so all four readers share one needle.
pub fn set_filter(fav_ui: &FavoritesUi, filter: &str) {
    *fav_ui.state().filter.lock() = row_match::fold_needle(filter);
}

/// Fetch the full favourites list, cache it under the current sort's display
/// order, then re-apply the filter. Runs on a tokio worker; the sort is resolved
/// here rather than in SQL — see [`resort_and_apply`].
pub async fn refresh_tracks(
    ...
    // Fetched in the query's fixed `sort_key` order; display order is derived in
    // memory below, so a cold fetch and a header click share one code path.
    //
    // Both ways of storing nothing re-arm the flag, the tab pick having
    // *consumed* it before spawning this: without the re-arm a failed query
    ...
    // A leave landing while the query was in flight has already wiped
    // `tracks_all` and emptied the model, so everything below would undo that
    // teardown behind a view nobody can see. Nothing is lost: every leave sets
    // `mark_dirty`, so the next enter re-fetches.
    if !fav_ui.section_active() {
        fav_ui.mark_songs_dirty();
        return Ok(());
    }

    // Warm the row covers before the first apply, or a cold cache pays one
    // synchronous decode per unique cover at paint time. Walked through the
    // display permutation so the prefix surviving the cap is what paints first;
    // pre-filter, which only diverges on a refresh taken with a filter already
    // narrowing past the cap, keystroke re-filters only narrowing a warm set.
    //
    ...
    // The decode burst is an `.await`, so the guard is asked again — after the
    // slow part, because before it the leave hasn't happened yet.
    if !fav_ui.section_active() {
        fav_ui.mark_songs_dirty();
        return Ok(());
    }

    // The Songs tab's chips, folded on the worker that already holds the rows —
    // which is why `publish_favorites` costs nothing. Outside the gate: what
    // must not straddle `release_section_state`'s wipe is the pair of *stores*,
    // so the walk has no business holding a lock the wipe and both sibling
    // fetches queue behind.
    let fold = crate::ui::hero_folds::fold_tracks(&rows);

    // A header click landing mid-fetch re-sorts the cache it finds and returns,
    // with no second query behind it to correct the order — so this store would
    ...
    // converts every favourite, so a click landing inside it leaves the header
    // naming one order and the list showing another until the next library tick.
    ...
    // `kick_full_refresh` runs this concurrently with the hero and grid fetches,
    // so whichever of those published did it against the previous fold.
...
/// Re-walk the cached `tracks_all` through the active filter and push the result
/// into `Favorites.tracks`. Entirely in memory. Selected ids are re-stamped
/// before the swap, so a filter change or library refresh doesn't visually drop
/// a selection — the row may shift index but keeps its checkbox and background.
///
/// A hidden section is never written to, the way
/// `grids::apply::write_filtered_grids` refuses to, and the check must sit in
...
/// `Favorites.tracks` feeds one element under `if tab-idx == tab-songs`, so on
/// the other two tabs every row built here reaches nothing —
/// [`build_filtered_tracks`]' check is what skips the cost, and
/// [`write_filtered_tracks`]' what stops a pick landing mid-post from pinning a
/// row per favourite behind a tab the user just left.
...
///
/// `grids::apply::apply_filtered_grids_now`'s twin, for the same reason:
/// `invoke_from_event_loop` posts even when called *from* the UI thread, so a
/// redraw can win the race — and the tab-leave emptied this model, so what a
/// lost race paints is a `TrackList` of headers over nothing.
...
///
/// Runs on the calling thread off one `Arc` snapshot and allocates nothing per
/// row — the cache holds converted rows, so a survivor is cloned, not rebuilt.
...
fn write_filtered_tracks(ui: &AppWindow, fav_ui: &FavoritesUi, mut rendered: Vec<UiTrackListRow>) {
    ...
    // A per-row rewrite where the identities align — a library tick — keeps the
    // `ListView`'s delegates instead of tearing them all down; a structural
    // change falls back to `set_vec`.
