...
//! reads a decoded cover and hands back a [`crate::themes::Palette`], so it is the adapter between
//! the two, and it is here rather than in `themes/` because a filter change in it moves every
//! generated colour and the resampler it must agree with is this tier's.
//!
