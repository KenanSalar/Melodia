...
Opus is not decoded yet ([#35](https://github.com/KenanSalar/Melodia/issues/35)). Because `.oga` and `.mka` are containers Melodia does scan, an Opus track inside one is still added to your library; it just won't play until that lands, and a Matroska one is listed without a duration, since that comes from the decoder.
