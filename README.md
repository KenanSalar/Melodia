...
- 64-band spectrum analyzer under the Now Playing artwork, tapped off the post-DSP audio and tinted to the album's own accent colour; bands are logarithmic across 50 Hz – 16 kHz (the equalizer's own range) so every bar covers the same musical interval, and it decays to rest on pause or can be switched off entirely
