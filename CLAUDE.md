...
- `melodia-ui/` — the workspace's second crate, and nothing but the Slint compiler's output: a build script, and a `lib.rs` holding `include_modules!` behind the blanket `#[allow]`. Split off so it is built **once** instead of once per compilation of `melodia` (which `cargo test` builds twice — rlib and `--test` harness), and so it rebuilds only when a `.slint` file or a translation catalog changes rather than on every Rust edit.
...
- **The Slint-generated code is its own crate, `melodia-ui`.** `melodia-ui/build.rs` runs the Slint compilation (the sized-thread wrapper, bundled translations); `melodia-ui/src/lib.rs` is the `include_modules!` block; `src/lib.rs` re-exports it with `pub use melodia_ui::*;`, so every call site still names the generated types as `crate::AppWindow`, `crate::TrackRow`, …. **Two things must stay in the root `build.rs`**: `load_dotenv()`, because `cargo:rustc-env` only reaches the crate whose build script emitted it (moving it silently ships keyless, ListenBrainz-only builds), and the Windows `winresource` icon embed, which embeds into the binary. The root manifest is still the workspace root, so `[patch.crates-io]` and `[profile.*]` keep applying; `winit/` carries its own `[workspace]` and is excluded besides.
...
- **Bundled translations.** `melodia-ui/build.rs` calls `with_bundled_translations("../translations")` + `with_default_translation_context(DefaultTranslationContext::None)`. Layout: `translations/<lang>/LC_MESSAGES/melodia-ui.po`. English is source baseline (no `en.po`). **The basename tracks the package name and isn't a free choice** — slint-build derives the gettext domain from `CARGO_PKG_NAME` with no override (`slint-build/lib.rs`, `compile_with_output_path`), so renaming the `melodia-ui` crate means renaming all six `.po` files with it or the build fails with "No translations found". Relative paths there resolve against `$CARGO_MANIFEST_DIR`, not the CWD, which is why `../` reaches the repo root.
...
- **Every compiling job builds in two phases, and that is what keeps CI inside the runner's 16 GB.** `cargo test` compiles this crate **twice, concurrently**: once as a plain rlib (the bin and the integration tests link it) and once as a `--test` harness. The two are independent, so cargo schedules them side by side, and with the ~390k-line generated `app-window.rs` in it **each rustc reaches ~10 GiB** — measured `11.07 + 9.89 = 20.96 GiB` on a cold build, against a 16 GB runner. Over the ceiling it does not fail, it **swaps**, and a 14-minute job silently runs for most of an hour looking like a hang (run `30271825979`, cancelled at 55 min). Phase 1 (`--lib`, `CARGO_BUILD_JOBS: 4`) builds the harness compilation alone, which also gets the ~600 dependencies compiled at full width — they are many small units and never the problem; phase 2 (`CARGO_BUILD_JOBS: 2`) then builds the rlib with the harness already done, so only one big rustc runs at a time. Measured **10.25 GiB** for the gate and **9.78 GiB** for coverage, both set by phase 1. **Keeping the two phases apart is the whole margin — don't collapse them back into one command.** A rustc's LLVM codegen threads draw jobserver tokens, so the job count scales a *single* process's peak too (8.58 / 10.04 / 11.55 GiB at jobs=1/2/4 in phase 2): `jobs=2` hides under phase 1's ceiling and is free, `jobs=4` makes phase 2 the ceiling for under a minute's gain, and `jobs=1` everywhere would cap memory but serialise all ~600 dependency compiles — which is exactly what the phase split exists to avoid. Don't substitute a cheap lever either; all were measured and none move this: `split-debuginfo=unpacked`, `debug=0` and dropping coverage instrumentation are ~0.5 GiB each, `codegen-units` does nothing across 16..4096 (and is worse at 1), and cutting test binaries does *nothing* (they are small crates linking a prebuilt rlib, not copies of the generated unit). The one real cure — getting the generated Slint unit out of the crate that gets compiled twice — **has since shipped as the `melodia-ui` crate**, so the generated `app-window.rs` is now built once per job rather than twice. **The two phases stay until someone re-measures**: the numbers above all predate the split, and `melodia`'s two compilations are now much smaller but still concurrent. Collapse them only against fresh peak-RSS figures, not on the assumption that the split was sufficient. `--tests` means *every* target with `test = true`, so phase 2 re-runs the lib tests too; duplicate profraw can't skew coverage, which is region-hit rather than hit-count.
