...
- **`stats_changed_tx` vs `library_changed_tx`.** Play-count flushes bump the stats channel only; Favorites is its sole subscriber (hero mosaic + Most Played rank by `play_count`). Everything structural (scans, watcher, imports, favorite toggles) stays on `library_changed_tx`.
- **Track projections by use case.** `TrackSummary` (12 cols; queue/NP/playback); `TrackListRow` (19; lists, incl. Files/Browse — it renders through the shared `TrackList` component, so no narrower browse-only slice); `TrackMeta` (8; NP chips); `Track` (40; scan ingest, hash backfill, detail, test fixtures). Each has a `*_columns()` helper. Pick the slimmest — `SELECT *` into `Track` for a list view costs ~30 unused `Option<String>` decodes/row.
