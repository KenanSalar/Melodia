...
- `ui/{albums,browse,tracks,queue_sheet}/` — per-view `Ui` handle. **`AlbumsUi`** releases grid covers on `open_album`, detail pair on `close_detail`. **`BrowseUi`** fetch uses stale-fetch token + library-folder-rooted breadcrumbs. **`TracksUi`**: `RowSearchKey` + `PreparedTrackRow` + `SectionState` like the entity grids — the boot `library_changed` refresher and Browse's subscriber are visibility-gated (hidden ⇒ mark dirty; `section-active-changed` re-enter ⇒ one deferred re-fetch), since play-count flushes bump `library_changed_tx` after every track completion. **`QueueSheetHandles`**: two-phase open + epoch-guarded teardown + `ShadowEntry`.
