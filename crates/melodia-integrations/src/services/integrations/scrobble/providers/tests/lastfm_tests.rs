...
fn sign_matches_known_vector() {
    ...
    assert_eq!(signature, format!("{:x}", md5::compute("api_keyKEYmethodauth.getTokenSECRET")));
...
fn now_playing_params_omit_signature_and_format() {
    ...
    assert_eq!(params.get("method").map(String::as_str), Some("track.updateNowPlaying"));
...
fn scrobble_params_use_bracketed_indices() {
    ...
    assert_eq!(params.get("method").map(String::as_str), Some("track.scrobble"));
    assert_eq!(params.get("artist[0]").map(String::as_str), Some("Artist A"));
    assert_eq!(params.get("track[1]").map(String::as_str), Some("Song B"));
    assert_eq!(params.get("timestamp[0]").map(String::as_str), Some("1700000000"));
...
fn love_params_select_method_by_flag() {
    ...
    assert_eq!(unloved.get("method").map(String::as_str), Some("track.unlove"));
}

#[test]
fn error_codes_classify_by_retryability() {
    assert!(matches!(classify(9, String::new()), LastfmError::InvalidSession));
    assert!(matches!(classify(11, String::new()), LastfmError::Transient { .. }));
    assert!(matches!(classify(16, String::new()), LastfmError::Transient { .. }));
    assert!(matches!(classify(29, String::new()), LastfmError::Transient { .. }));
    assert!(matches!(classify(6, String::new()), LastfmError::Api { code: 6, .. }));
}
