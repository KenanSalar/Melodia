...
///
/// A redaction test builds its fixture by calling this rather than spelling `/home/testuser`
/// under a faked `$HOME`: `dirs::home_dir()` asks Win32 for the profile folder and never reads
/// the environment, so the faked form is a fixture only Linux can honour and every such test
/// was red on Windows. The Unix `$HOME` arm is pinned separately, by this module's own tests,
/// which is where a faked variable still belongs.
