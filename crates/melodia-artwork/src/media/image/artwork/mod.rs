...
pub use composite::{compose_artwork, compose_cover};
...
pub fn store_image(bytes: &[u8], ext: &str, dir: &Path) -> Option<String> {
    if bytes.is_empty() {
        return None;
    }
...
pub fn cache_image_file(source_path: &Path, artwork_dir: &Path) -> Option<String> {
