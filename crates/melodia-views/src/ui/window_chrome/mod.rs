...
/// their session: past `quit_event_loop()` the window is gone, and a failed `exec` in the
/// binary's `shutdown::respawn_if_requested` has nothing to fall back to. Hence resolving
