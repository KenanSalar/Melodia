...
//! for one. Only the mounted tab's model, and only while the section is on
//! screen — the two grids are mutually exclusive, so the hidden one is
...
//! names which prepared hash belongs to the tab on screen, and [`sort`] owns the
//! Favorite Artists order.
...
pub use apply::apply_filtered_grids_now;
