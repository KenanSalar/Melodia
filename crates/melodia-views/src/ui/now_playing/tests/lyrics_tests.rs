...
fn a_short_line_takes_one_row() {
    assert_eq!(wrapped_lines("short line", TYPICAL_WIDTH), 1);
}
...
fn a_blank_line_still_takes_a_row() {
    ...
    assert_eq!(wrapped_lines("", TYPICAL_WIDTH), 1);
    assert_eq!(wrapped_lines("   ", TYPICAL_WIDTH), 1);
}

#[test]
fn a_line_past_one_row_takes_two() {
    assert_eq!(wrapped_lines(&"x".repeat(80), TYPICAL_WIDTH), 2);
}

#[test]
fn a_line_long_enough_to_be_a_paragraph_is_capped() {
    // Past the cap the panel would scroll more than it shows.
    assert_eq!(wrapped_lines(&"x".repeat(4_000), TYPICAL_WIDTH), MAX_WRAPPED_LINES);
}
...
fn a_width_nobody_has_reported_yet_is_survivable() {
    ...
    assert_eq!(wrapped_lines("anything", 0.0), 1);
    assert_eq!(wrapped_lines("anything", -5.0), 1);
}
