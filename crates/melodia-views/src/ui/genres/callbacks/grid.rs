...
pub(super) fn wire(
    ...
    if let Some(sort) = persisted_sort(view_state, view_id::GENRES) {
        genres.set_sort_field(SharedString::from(sort.field.as_str()));
        genres.set_sort_dir(SharedString::from(sort.dir.as_str()));
    }
