fn main() {
    ...
    //
    // `cargo:rustc-env` only reaches the crate whose build script emitted it, so
    // this has to sit with the `option_env!` sites. They are still in this
    // package; it follows them into `melodia-integrations` when they leave.
    load_dotenv();
