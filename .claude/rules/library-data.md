...
- **The artwork sweep runs *after* that tx commits, never inside it** (`tasks::artwork_sweep`,
  ...
  `media::artwork` writes, and nothing in the reference set may name it. **That set is five
  columns** — `tracks.artwork_path`, `albums.artwork_path`, `artists.image_path`,
  **`playlists.thumbnail_path`** and **`radio_stations.artwork_path`**. The last two are the ones
  that bite, each reachable through no other row: dropping the playlist arm blanks every custom
  mosaic, and dropping the radio arm deletes a station logo that came off a third-party host and
  can never be re-derived. A one-hour grace window covers the file a tag edit or scan worker has
