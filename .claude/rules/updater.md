...
- **Replacing the binary under a running process breaks its own path two different ways, and only one of them is recoverable after the fact.** An RPM/DEB upgrade *unlinks* `/usr/bin/Melodia`, and the kernel then appends a literal `" (deleted)"` to `/proc/self/exe` — which `std::env::current_exe()` hands back verbatim. That one is resolved centrally by **`services::current_exe`** (strip the marker, but only when the suffixed path is dead and the stripped one is live, so a file genuinely named that keeps its own path), and `install_target()` is the single updater consumer — which is what also keeps the marker out of `desktop_integration`'s `Exec=` line and out of `linux_pkg::detect`'s package-DB lookup. The atomic swap *renames* instead, to `<target>.old`, and a rename leaves `/proc/self/exe` reporting the stale path with a straight face — nothing can undo that after the fact, which is why **`ui::window_chrome::set_respawn_exe`** still has to capture the target *before* the swap and stays load-bearing. Don't read the central fix as retiring it. The same asymmetry is why cargo re-uplifting `target/debug/Melodia` (unlink, then hardlink from `deps/`) puts a dev build in the first case: the inode's mtime never moves, so the file looks untouched while every running process's path is marked deleted.
